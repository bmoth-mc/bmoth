...
public class EqualAstTest extends TestParser {
    ...
    public void testEqualAstIfSubstitution() {
        MachineNode machine = new MachineBuilder()
            ...
            .addOperation("op_not_if = SELECT a = 3 THEN a:=2 END")
            .build();
        ...
        assertFalse(substitutions.get(0).equalAst(substitutions.get(5)));
    }

    @Test
    public void testEqualAstSelectSubstitution() {
        MachineNode machine = new MachineBuilder()
            .setName("IfSubstitutionMachine")
            .setVariables("a")
            .setInvariant("a : INTEGER")
            .setInitialization("a := 0")
            .addOperation("op1 = SELECT a : {2+1} THEN a:=2 END")
            .addOperation("op2 = SELECT a : {2+1} THEN a:=2 END")
            .addOperation("op_changed1 = SELECT a = 3 THEN a:=2 END")
            .addOperation("op_changed2 = SELECT a : {2+1} THEN a:=1+1 END")
            .addOperation("op_not_select = IF a = 3 THEN a:=2 ELSE a:=3 END")
            .build();
